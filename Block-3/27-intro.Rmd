# Change & accumulation {#change-accumulation}

<div style="float:right;">[![](www/icons/edit.png)](https://github.com/ProjectMOSAIC/MOSAIC-Calculus/blob/main/Block-3/27-intro.Rmd)</div>
...
```{r pop-graph, echo=FALSE, fig.cap="US population since 1790. Left: linear axes. Right: semi-log axes", fig.margin=TRUE}
...
If the trend in the growth rate continues, the US will reach zero net growth about 2070, then continue with negative growth. Of course, negative growth is just decline. A simple prediction from Figure \@ref(fig:pop-growth) is that the argmax of the US population---that is, the year that the growth rate reaches zero---will occur around 2070.
...
2. The name of the with-respect to variable is contained in the "dx" at the end of the notation: $\int f(x) d\color{magenta}{x}$
...
`r insert_calcZ_exercise("27.02", "bcJnUq", "Exercises/wolf-beat-book.Rmd")`

`r insert_calcZ_exercise("27.04", "Ya2cVq", "Exercises/fir-lead-canoe.Rmd")`

`r insert_calcZ_exercise("27.06", "wmyERp", "Exercises/bee-bid-knife.Rmd")`


`r insert_calcZ_exercise("27.08", "kEAW8g", "Exercises/dog-have-mattress.Rmd")`

`r insert_calcZ_exercise("27.10", "NYPDR", "Exercises/horse-takes-pillow.Rmd")`
