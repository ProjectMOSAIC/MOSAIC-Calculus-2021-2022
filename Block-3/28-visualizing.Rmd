...
Pts <- Znotes::segmentize(cos(2*pi*x/10) ~ x, h=0.5, domain(x=c(-5,5)), npts=100)
...
Pts <- Znotes::segmentize(F(x) ~ x, h=0.25, domain(x=c(-5,5)), npts=20)
P1 <- 
...
Each of those straight-line segments is drawn over a domain $x_i < x < x_i+dx$ that has width $dx$. For $dx$ small enough, the segment is well approximated by a straight line with slope  $\partial_x F(x_i)$. Multiplying slope by width gives the segment height: $h \partial_x F(x_i)$. 
...
Pts <- Znotes::segmentize(cos(2*pi*x/10) ~ x, h=0.5, domain(x=c(-5,5)), npts=1) %>%
