...
Now, consider a problem like $\int x \cdot \cos(x) dx$ that we don't yet know how to solve. Let's associate this problem with the left side of the parts re-arrangement equation. With luck, we will recognize a problem that we'll know how to do on the right-hand side. 
...
According to this proposed splitting, we have $u(x) = x$ and $v'(x) dx = \cos(x) dx$. To plug things into the right side of the parts re-arrangement we'll need to find $v(x)$ and $u'(x) dx$. Since we know $u(x) = x$ it's easy to take the differential, 
...
Now that we know the $v(x)$ that's consistent with our original splitting of the anti-derivative into $\int u(x) \cdot v'(x) dx$ we can plug in our results to the right side of the parts re-arrangement equation:

$$\int x \cdot \cos(x)dx = x \sin(x) - \int \underbrace{\sin(x)}_{v(x)}\  \underbrace{\ 1\ dx\ \strut}_{u'(x) dx}$$
We're in luck, because we already know the anti-derivative $\int \sin(x) dx = -\cos(x)$, giving us the result 
...
`r insert_calcZ_exercise("31.03", "HoQaaq", "Exercises/owl-give-closet.Rmd")`

`r insert_calcZ_exercise("31.05", "1qDQZz", "Exercises/bird-hold-jacket.Rmd")`

`r insert_calcZ_exercise("31.07", "YMuG92", "Exercises/chicken-cost-coat.Rmd")`

`r insert_calcZ_exercise("31.08", "Y7ecmI", "Exercises/frog-pitch-clock.Rmd")`

`r insert_calcZ_exercise("31.09", "LPlcgV", "Exercises/lamb-mean-pan.Rmd")`

`r insert_calcZ_exercise("31.10", "v1ktnB", "Exercises/wolf-speak-oven.Rmd")`

`r insert_calcZ_exercise("31.11", "gzLEo6", "Exercises/aspen-give-canoe.Rmd")`

`r insert_calcZ_exercise("31.12", "4vJNnj", "Exercises/rhinosaurus-beat-screen.Rmd")`

`r insert_calcZ_exercise("31.13", "LpYZy4", "Exercises/crow-eat-bottle.Rmd")`

`r insert_calcZ_exercise("31.14", "Wo7Elt", "Exercises/pine-sing-scarf.Rmd")`

`r insert_calcZ_exercise("31.15", "A0u94a", "Exercises/shark-dive-hamper.Rmd")`

`r insert_calcZ_exercise("31.16", "xVaQOh", "Exercises/wolf-jump-sofa.Rmd")`

`r insert_calcZ_exercise("31.17", "kay2zx", "Exercises/pine-fight-painting.Rmd")`

`r insert_calcZ_exercise("31.19", "nef1lB", "Exercises/cat-go-window.Rmd")`

<!-- table of integrals --> 
`r insert_calcZ_exercise("31.22", "f6C4Ia", "Exercises/wolf-come-room.Rmd")`
