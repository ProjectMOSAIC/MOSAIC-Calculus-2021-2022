...
See diffusion illustration at <https://en.wikipedia.org/wiki/Fick%27s_laws_of_diffusion>
...
> Essay question: *A more fundamental way to measure the increase in width of $C(x, t)$ with time is to use the **variance** rather than the standard deviation. Describe briefly the very simple pattern that the variance of $C(x, t)$ follows over time. (You don't have to give a formula, just say what function it is.)*
...
draw_pipe <- function(time=1, sigma = 1.7, 
  ...
  slice_plot(f(x) ~ x, domain(x=-25:25), npts=500) %>%
...
set.seed(101)
draw_pipe(time=1, sigma = 1.7, n=1000)
set.seed(102)
draw_pipe(time=10, sigma = 4.5, n=1000)
set.seed(103)
draw_pipe(time=20, sigma = 6.4, n=1000)
set.seed(104)
draw_pipe(time=40, sigma = 9.0, n=1000)
```
