...
Consider the following situation. A space probe is being acted on by a constant force, as in Figure \@ref(fig:probe-constant).

```{r probe-constant, echo=FALSE, fig.cap="A space probe accelerating in a straight line, but observed from a distant planet."}
Arc1 <- tibble(
...
P1 <- gf_point(1.9 ~ .9, size=3, color="orange") %>%
  gf_text(1.9 ~ .9, label="t=10", hjust=-1, color="blue") %>%
...
Finding the location $y(t)$ of the probe along the dashed line is easy, solve $$F = m\, \partial_{tt}\, y(t)$$ giving $$\partial_t y(t) = \frac{F}{m}t \ \ \implies\ \ \ y(t) = \frac{1}{2} \frac{F}{m} t^2$$

For the person on the planet, tracking the probe with a telescope, the angle $\theta(t)$ varies with time. Of course, the angle will always correspond with the $(x,y)$ position of the probe: $$\theta(t) = \arctan\left({\large\strut} \frac{y(t)}{x(t)}\right)\ .$$


HOW does theta accelerate????
