...
<div style="float:right;">[![](www/icons/edit.png)](https://github.com/ProjectMOSAIC/MOSAIC-Calculus/blob/main/Block-4/B4-mechanics.Rmd)</div>
...
```{r probe-constant-orig, eval=FALSE, echo=FALSE, fig.cap="A space probe (orange) accelerating in a straight line, but observed as a change in angle $\theta$ seen from a planet (green)."}
Arc1 <- tibble(
...
Three points in the diagram describe a right triangle: 
 the probe's position at $t=0$, the center of the planet, and the probe's position at time $t$. The length of the horizontal leg of the triangle is $D(t=0)$ which not a function of time, so we'll
...
The angular coordinate representation is helpful when studying the ***rotation*** of objects. To illustrate, imagine a different configuration for the system than that in in Figure \@ref(fig:probe-constant) where the space probe was free to accelerate in a straight line. Instead, suppose the rocket is mounted on a carousel, that is a wheel whose axle goes through the center as in Figure \@ref(fig:rocket-on-disk).

```{r rocket-on-disk, echo=FALSE, fig.cap="The space probe mounted on a rigid wheel that can spin around its center.", out.width="50%"}
