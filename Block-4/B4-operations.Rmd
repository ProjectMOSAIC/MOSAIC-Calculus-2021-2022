...
<div style="float:right;">[![](www/icons/edit.png)](https://github.com/ProjectMOSAIC/MOSAIC-Calculus/blob/main/Block-4/B4-operations.Rmd)</div>
...
A simple equation like $3 + 4 = 7$ is a statement of fact: three plus four is indeed exactly the same as seven. But $x^2 - 4 x + 3 = 0$ is not a fact. The equality might be true or false, depending on what $x$ happens to be. In an algebra course, the equation is intended to be an instruction to a person: 
...
$$\text{solve_step}(z) \equiv z + \frac{v-f(z)}{\partial_x f(z)}\ .$$
...
$$x_1 = \text{solve_step}(x_0)\\
x_2 = \text{solve_step}(x_1)\\
x_3 = \text{solve_step}(x_2)\\
\vdots\\
x_{i+1} = \text{solve_step}(x_{i})\\\ \\
...
slice_plot(f(x) ~ x, domain(x=-2:5), color="brown", label_text="f(x)", label_vjust=-0.5) %>%
  ...
  gf_refine(scale_x_continuous(breaks = c(xstar, x0, 0), labels = ticks, minor_breaks = NULL), coord_fixed(ratio=1/5)) +
...
$$\text{solve_step}(z) = z - \frac{z^2 - z - 4}{2 z - 1}$$
```{r echo=FALSE}
f <- makeFun(x^2 - x ~ x)
...
`r {x0 <- 2} |> oneline_block(width="30%", phrase="", inline=FALSE)`     <br>

`r {x1 <- solve_step(x0)} |> oneline_block(width="30%", digits=8)`
`r {f(x1)} |> oneline_block(width="20%", digits=8)`
`r {x2 <- solve_step(x1)} |> oneline_block(width="30%", digits=8)`
`r {f(x2)} |> oneline_block(width="20%", digits=8)`
`r {x3 <- solve_step(x2)} |> oneline_block(width="30%", digits=8)`
`r {f(x3)} |> oneline_block(width="20%", digits=8)`
...
`r {f <- makeFun(x^2 - x ~ x)} |> oneline_block(width="70%", phrase="", inline=FALSE)` <br>

Construct the take-a-step function:

`r {take_step <- make_solve_step_fun(f(x) ~ x, v=4)} |> oneline_block(width="70%", inline=FALSE, phrase="")` <br>

Take three steps starting at $x_0 = 3$:

`r {x0 <- 3} |> oneline_block(width="50%", phrase="", inline=FALSE)` <br>

`r {x1 <- take_step(x0)} |> oneline_block(width="50%")`
`r {x2 <- take_step(x1)} |> oneline_block(width="50%")`
`r {x3 <- take_step(x2)} |> oneline_block(width="50%")`
`r {f(x3)} |> oneline_block(width="20%")`
...
We'll illustrate by solving $x^3 = 6$ for $x$.

`r {Zeros(x^3 - 6 ~ x, domain(x=c(1,6)))} |> oneline_block(width="60%")`
...
We'll solve this equation for $x$ and, having in mind the iterative process of the previous section, call the result $x_1$
...
`r {f <- rfun(~ x, 3215)} |> oneline_block(inline=FALSE, phrase="")`


```{r echo=FALSE, out.width="50%", fig.align="center"}
Xstar <- argM(f(x) ~ x, domain(-5:5))
...
`r {argM(f(x) ~ x, domain(x = -5:5))} |> oneline_block(width="50%")`
...
`r insert_calcZ_exercise("32.03", "THpUtP", "Exercises/aspen-dig-ship.Rmd")`

`r insert_calcZ_exercise("32.05", "ZRYTU3", "Exercises/water-sink-ship.Rmd")`

`r insert_calcZ_exercise("32.07", "36xJU8", "Exercises/fish-ring-door.Rmd")`

`r insert_calcZ_exercise("32.09", "6bX0Zx", "Exercises/kid-rise-coat.Rmd")`

`r insert_calcZ_exercise("32.11", "gCKs9N", "Exercises/birch-wake-linen.Rmd")`

`r insert_calcZ_exercise("32.13", "JBw8oV", "Exercises/snake-get-ring.Rmd")`
