...
We'll illustrate how we want to work with probability with a simple game of the sort you might encounter at a fund-raising fair. There is a jar on display, filled with coins that have been donated by one of the fair's sponsors. You pay $1 (which goes to a good cause) to enter the contest. Your play is to describe how much money is in the jar, writing your description down along with your name on an entry form. At the end of the day, an official will open the jar, count the money, and announce who made the best estimate. The winner gets the money in the jar.

```{r echo=FALSE, out.width="15%", fig.align="center"}
knitr::include_graphics("www/jar-coins.png")
```

Were this an ordinary fair, your entry form might list the amount of money that you think is in the jar, say 42 dollars. The winner would be determined by seeing whose guess was closest to the actual value of the coins in the jar. Simple.

But this is a calculus fair and the game is constructed to illustrate some basic concepts of how to represent your knowledge and uncertainty in a reasoning way, for instance how to combine new evidence with existing knowledge. The evidence is what you can see of the coins in the jar. Another form of evidence might be your sense of the heft of the jar when you pick it up, or a measurement of the height and circumference of the jar. Your existing knowledge might be a statement from the contest organizers that only legitimate coins are in the jar; for instance, that there's not a tennis ball in the center of the jar or a packet of washers from the hardware store. 

The contest form states: "On the graph-paper axes below, sketch a continuous function expressing your best belief about how much money is in the jar. At all inputs, the function value must be zero or greater."
...
Take a minute to look at the picture of the jar and draw your function on the axes shown above. We want to see how you do compared to the following three contest entries.

```{r contest-entries, echo=FALSE, fig.cap="Three contestants' contest entries.", warning=FALSE}
P1 <- Money_graph %>% gf_labs(title="Johnny Milton") %>%
  slice_plot(ifelse(mpg > 22 & mpg < 30, 5, 0) ~ mpg, 
             domain(mpg = 0:100), size=2, npts=300) %>%
  gf_vline(xintercept=~32, color="brown", alpha=0.3)
P2 <- Money_graph %>% gf_labs(title="Louisa Alcott") %>%
  slice_plot(5 ~ x, domain(x=0:100), size=2) %>%
  gf_vline(xintercept=~32, color="brown", alpha=0.3)
P3 <- Money_graph %>% gf_labs(title="Geoff Chaucer") %>%
...
You might notice that all three functions $f_\text{Johnny}()$, $f_\text{Louisa}()$, and $f_\text{Geoff}()$ have output values that are sometimes numbers greater than one. But even if we relabed the vertical axis to replace 5 with, say, 0.5, the normalized versions of the function still cannot produce an output that is a probability. The problem is that the normalized versions are scaled by a constant $A$ that is not dimensionless. Note from the graph that $\int f(M) dM$ has units of *dollars*: the vertical axis is a dimensionless number, but the horizontal axis is in dollars. The units of $A$ will therefore be in dollars and there is no such thing as a probability of 0.066 per dollar.

We call the functions probability **density** functions because ...
