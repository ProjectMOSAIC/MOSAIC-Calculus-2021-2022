...
To illustrate, let's return to the jar-of-money contest and the three competitors' entries as shown  in Figure \@ref(fig:contest-compare). For convenience, that Figure is reproduced here:

```{r jar_functions2, echo=FALSE, fig.cap="The contest entries shown in Figure \\@ref(fig:contest-compare)."}
Jar_functions %>%
...
The output of ${\cal L}_{\$32.14}(\text{contestant})$ is the probability density assigned by the contestant to the observed value $32.14. You can read this from Figure \@ref(fig:jar-functions2). For your convenience, the observation $32.14$  has been annotated with a faint brown vertical line. 
...
We can't know which of the three priors in Figure \@ref(fig:beta-shapes) is most appropriate. After all, until rich enough data become available, each prior is just an opinion. So we'll repeat the update-with-data analysis for each of the three priors. If, in the end, the results from the three priors substantially agree, then we can conclude that the data is shaping the results, rather than the prior.
...
1. The probability of a + test in a group of people *with* the disease. We'll call this $p_d(+)$.
2. The probability of a - test in a group of people *without* the disease.  We'll call this $p_h(-)$.
...
In a real-world situation, we would have to do some experiments to measure $p_h(-)$ and $p_d(+)$. For our example we'll set $p_d(+) = 0.8$ and $p_h(-) = 0.7$.
