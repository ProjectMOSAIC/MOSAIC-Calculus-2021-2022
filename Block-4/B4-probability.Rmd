...
For instance, the mean is the expectation value of $x$, so the mean of a quantity with a gaussian distribution is 
...
In this section, we'll examine the accepted technique for combining data with probability density functions in order to combine previous knowledge with new observations. The technique, called ***Bayesian inference***, is used throughout science and engineering. (Surprisingly, satisfactory computer algorithms to do the calculations were invented only in the last 40 years---so this is a modern topic.) 

Because there are several subtle aspects to Bayesian inference, we will present the techniques in terms of two specific examples. For most people, Bayesian inference is a new approach to thinking. It can take years of experience to develop a confident understanding. But you have to start somewhere .... 



that can take years
You'll see how "knowledge" can be represented as a probability density function, and the combination of new data with prior knowledge involves simple multiplication to create a relative density function that can be normalized into the probability density function that represents our new knowledge, after we've incorporated the data.
