...
# Starting on routine for quad splines with arbitrary number
# of segments.
makeM <- function(nsegs=4, free=0, data) {
  Ones <- diag(nrow=nsegs)
  top <- cbind(Ones, Ones)
  left <- matrix(c(1,-1, rep(0, nsegs-1)), nrow=nsegs,ncol=nsegs, byrow=TRUE)
  bottom <- cbind(left, 2*Ones)
  rbind(top, bottom)
}
...
quad_spline <- function(free = 0, data = Pts) {
  ...
  a2 <- Pts$x[2]
  a3 <- Pts$x[3]
  b <- cbind(c(Pts$x[2:4] - Pts$x[1:3], c(0, 0, free)))
  x <- qr.solve(M, b)
  p1 <- makeFun(a1 + b1*(t-1) + c1*(t-1)^2 ~ t,
                b1 = x[1], c1 = x[4])
  p2 <- makeFun(a2 + b2*(t-2) + c2*(t-2)^2 ~ t,
                b2 = x[2], c2 = x[5])
  p3 <- makeFun(a3 + b3*(t-3) + c3*(t-3)^2 ~ t,
                b3 = x[3], c3 = x[6])
  makeFun(ifelse(t < 2, p1(t),
                    ifelse(t < 3, p2(t), p3(t))) ~ t)
}
...
```{r letter-e, echo=FALSE, fig.cap="The outline of a letter in a computer font is often specified by a series of knot points (red dots). The path passes smoothly through some of the knot points, but has a discontinuous derivative at others.  The shape of the segment between two consecutive knot points is set by a \"control point\" (marked as x).", out.width="50%", fig.align="center"}
knitr::include_graphics("www/bezier-e.png")
```


<iframe width="350" height="370" src="b.svg?p=0.3,0.2,0.5,0.5,0.8,0.4,0.9,0.7&d=0.5,0.2,0.1,0.5,0.1,0.4,0.9,0.7&animate=1"></iframe>  

<iframe width="350" height="370" src="b.svg?p=0.3,0.2,0.5,0.5,0.8,0.4,0.9,0.7&d=-10,-10,-10,-10&animate=1"></iframe>  
...
Another example ... In [computer-aided design](https://en.wikipedia.org/wiki/Computer-aided_design) (CAD) systems, curved shapes are usually described by a person's placement of isolated points which are then automatically combined into a smooth form. 
