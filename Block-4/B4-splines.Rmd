...
Putting together the $x(t)$ and $y(t)$ interpolating functions, each of which has that extremum between knot points, leads to an absurdly complicated path, as seen in Figure \@ref(fig:quad-robot-points).


```{r quad-robot-points, fig.cap="Connecting the robot-path knots with a piecewise quadratic polynomial, constructed to be $C^1$. The path is pretty perhaps, but absurd.", echo=FALSE}
nudges <- rep(40, 16)
nudges[c(3,12)] <- -40
...
<p align="center"><iframe width="540" height="540" src="b-new.svg?a=0.1,0.1,0.2,0.9,0.6,0.2,0.9,0.2&b=-100,-100,-100,-100&c=-100,-100,-100,-100&animate=0"></iframe></p>
...
Since $x(t)$ and $y(t)$ can be interpreted as the $(x,y)$ coordinate at time $t$ of a pen on the drawing canvas, it's reasonable to think about the pen in physical terms.
...
slice_plot(xfun(t) ~ t, domain(t=0:1)) %>%
  gf_refine(coord_fixed())
```
