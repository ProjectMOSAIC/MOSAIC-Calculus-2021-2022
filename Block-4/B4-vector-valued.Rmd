...
The functions we have worked with up to now have **outputs** that are single quantities: e.g., height or speed or fuel consumed. In Section \@ref(cubic-spline), in order to present the programmed position versus time of a robot arm, we needed to construct two functions, one giving the x-coordinate of the arm, the other giving the y-coordinate. Using differentiation with respect to time, we can calculate from the x- and y-position functions the x- and y-velocity functions and the x- and y-acceleration functions. All six functions are shown in Figure \@ref(fig:robot-functions).
...
```{r robot-functions, echo=FALSE, fig.cap="The position, velocity, and acceleration of the robot arm as a function of time. The x-coordinate is the left column, the y-coordinate the right column."}
...
Relating the various functions in Figure \@ref(fig:robot-functions) is, at best, difficult. So difficult, in fact, that you may have trouble perceiving any relationships between the x- and y-functions or between, say, velocity and position.
...
To illustrate, consider the movement of the robot arm. In particular, we'll look at both velocity $\partial_t\, pos(t)$ and acceleration $\partial_{tt}\, pos(t)$.  Figure \@ref(fig:robot-functions) shows this as four distinct graphs, but Figure \@ref(fig:robot-vectors) does it in just a single graph.

```{r robot-vectors, echo=FALSE, fig.cap="A vector presentation of the velocity (blue) and acceleration (magenta) of the robot arm as functions of time."}
set.seed(102)
Vecs <- tibble(
  t = seq(1, 15, by=1),
