...
::: {.underconstruction}
Chapter not yet released[...](https://github.com/ProjectMOSAIC/MOSAIC-Calculus/blob/main/Block-5/B5-Vectors.Rmd)

Note: Use `\mathit{}` to set matrices: $\mathit{M}$. The point of this is to have something that works directly in the editor and does nice typesetting in the formatted document. `\origmathit{}` can be used to get the original italics behavior in the formatted document.
:::
...
Vectors embedded in three-dimensional space are central to physics and engineering. Quantities such as force, acceleration, and velocity are properly represented not as simple numerical quantities but as vectors with ***magnitude*** (that is, length) and direction. The statement, "The plane's velocity is 450 miles per hour to the north-north-west" is perfectly intelligible to most people, describing as it magnitude and direction. Note that the vector velocity can be understood without having to know where the plane is located; vectors have only the two qualities of magnitude and direction. Position is irrelevant to describing velocity, or, for that matter, force or acceleration.
...
rbind(1,3,-4)
```
Note that the elements are separated by commas in the ordinary way for R functions that take more than one input. 

Later in this block, we will be using data frames to define vectors. We'll introduce the R syntax for that when we need it. 

Note to R experts: The R language has a data structure called a "vector," which is a set of elements *without* the information needed to consider it a row or column vector. As such, the native R "vector" is not suited for linear-algebra computations in R.  

Many people construct mathematical vectors using the `matrix()` function. Such a `matrix()` command to produce a column vector would look like `matrix(c(1, 3, -4), ncol=1)`. This is a professional practice, but we regard this as too verbose for our purposes in this book, so we will use `rbind()` instead. Note that combining `rbind()` with `c()` or other preconstructed R "vectors" will not produce a mathematical row vector
```{r}
rbind(c(1,2,3))
rbind(1:5)
```
...
```
It's also possible to use `cbind()` and `rbind()` in the reverse order, so that the construction statement looks like the matrix itself.
```{r}
rbind(
    cbind(1, 10),
    cbind(2, 11),
...
This section introduces the arithmetic of vectors. With this arithmetic in hand, we can carry out the three tasks (and more!) on vectors that consist of a column of numbers. Especially noteworthy is that the arithmetic enables to to apply simple geometrical concepts to vectors in three or more dimensions.
...
This is perfectly ordinary multiplication applied component by component. 
...
$$\vec{v} \vec{w} = \left[\begin{array}{r}6\\2\\-4\\\vdots\\1\\8\end{array}\right]\  \left[\begin{array}{r}-3\\1\\-5\\\vdots\\2\\5\end{array}\right] = \left[\begin{array}{r}-18\\2\\20\\\vdots\\2\\40 \end{array}\right]$$
...
In principle, you can invoke the `+`, `-`, `*`, `/`, and `^` operations using ***functional*** notation. Nobody does this because the commands are so ugly:
```{r}
`+`(3, 2)
`/`(7, 4)
`-`(6, 2)
...
To the student encountering the dot product for the first time, a natural response is to wonder what such a two-step operation might be good for. As we walk through this block and the next one, you'll see the dot product playing a central role.
...
The arithmetic used to calculate the length of a vector is based on the Pythagorean theorem. For a vector $\vec{u} = \left[\begin{array}{c}4\\3\end{array}\right]$ the vector is the hypotenuse of a right triangle with legs of length 4 and 3 respectively. Therefore, $$\len{\vec{u}} = \sqrt{4^2 + 3^2} = 5\ .$$
...
$$\len{\vec{u}} = \sqrt{\strut\vec{u} \bullet \vec{u}}\ .$$
Although length has an obvious physical interpretation, in many areas of science including statistics and quantum physics, the ***square length*** is a more fundamental quantity. The square length of $\vec{u}$ is simply $\len{\vec{u}}^2 = \vec{u}\bullet \vec{u}$.
...
In statistics, applications if linear algebra often involve a simple constant vector, which we'll write $\vec{1}$. It is simply a column vector of 1s, 
...
The quantity $\vec{x} - \bar{x}$ is an example of ***scalar*** subtraction, which is done on a component-wise basis. For instance, with $$\vec{x} = \left[\begin{array}{r}1\\2\\3\\4\\\end{array}\right]$$ then $\bar{x} = 2.5$. This being the case, $$\left[\begin{array}{r}-1.5\\-0.5\\0.5\\1.5\\\end{array}\right]\ ,$$
...
`r insert_calcZ_exercise("41.XX", "IpAGZw", "Exercises/fir-feel-bowl.Rmd")`

`r insert_calcZ_exercise("41.XX", "Of7QlW", "Exercises/snake-choose-ring.Rmd")`

`r insert_calcZ_exercise("41.XX", "dwALW7", "Exercises/octopus-sell-mattress.Rmd")`

`r insert_calcZ_exercise("41.XX", "xuENab", "Exercises/rhinosaurus-break-knob.Rmd")`

`r insert_calcZ_exercise("41.XX", "MBTGQt", "Exercises/duck-do-pantry.Rmd")`

`r insert_calcZ_exercise("41.XX", "Q2ars0", "Exercises/squirrel-hear-glasses.Rmd")`

`r insert_calcZ_exercise("41.XX", "OoPatc", "Exercises/dog-let-coat.Rmd")`

`r insert_calcZ_exercise("41.XX", "Gw42pX", "Exercises/beech-iron-clock.Rmd")`

`r insert_calcZ_exercise("41.XX", "1Jxboc", "Exercises/snake-ride-glasses.Rmd")`

`r insert_calcZ_exercise("41.XX", "N98zli", "Exercises/falcon-begin-door.Rmd")`

`r insert_calcZ_exercise("41.XX", "PCfYDo", "Exercises/crocodile-talk-mattress.Rmd")`

`r insert_calcZ_exercise("41.XX", "4QzeWP", "Exercises/cat-do-pants.Rmd")`

`r insert_calcZ_exercise("41.XX", "5zmmBu", "Exercises/girl-send-scarf.Rmd")`

`r insert_calcZ_exercise("41.XX", "7KJQQd", "Exercises/bear-fight-closet.Rmd")`

`r insert_calcZ_exercise("41.XX", "k5u7hG", "Exercises/titmouse-stand-pantry.Rmd")`
