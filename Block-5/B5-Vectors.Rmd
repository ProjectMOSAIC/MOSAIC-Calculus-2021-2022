...
There are three vector tasks have familiar geometric properties. Given a set of vectors drawn on paper, you can carry them out by pencil assisted by a simple ruler and a protractor.
...
The ***dot product*** is one approach to multiplication of one vector with another. The dot product between $\vec{v}$ and $\vec{w}$ is written $$\vec{v} \bullet \vec{w}$$. (We'll use this big dot for a while, until you get used to seeing dot products. Then we'll make the dot smaller so that it draws less attention.)
...
a %dot% b
```
The vectors being combined with `%dot%` must both have the same number of elements. Otherwise, an error message will be given:
```{r error=TRUE}
...
The arithmetic used to calculate the length of a vector is based on the Pythagorean theorem. For a vector $\vec{u} = \left[\begin{array}{c}1\\2\end{array}\right]$
...
```{r}
Znotes::gvec(from=c(-2,3), to=c(4,4), color="red") %>%
Znotes::graph_paper(xticks=-5:5, yticks=-5:5) %>%
    Znotes::gvec(from=c(0,0), to=c(-2,3))
```
