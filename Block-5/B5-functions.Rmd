...
A central strategy in calculus is to break complicated objects  into simpler components. The target problem breaks up the target vector $\vec{b}$ into a linear combination of the vectors in a matrix $\mathit{A}$. There's no requirement that the individual vectors in $\mathit{A}$ be "simpler" than $\vec{b}$, but often the vectors in $\mathit{A}$ can be traced back to some easy-to-interpret source, such as a column in a data frame.
...
An important strategy for modeling such complicated oscillations is to ***decompose*** them into simpler parts. Here, those simple parts will be sinusoids and we'll model the waveform as a linear combination of many sinusoids, each with a particular period. This decomposition can be accomplished by solving the ***target problem*** $\mathit{A}\vec{x} = \vec{b}$, where $\vec{b}$ is the waveform, $\mathit{A}$ is the collection of sinusoids, and the coefficients $\vec{x}$ will become our new representation of the waveform.
