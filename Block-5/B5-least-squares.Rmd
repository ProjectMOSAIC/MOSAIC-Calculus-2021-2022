...
The `Car_facts` data frame shown below records several aspects of the design and performance of some classic cars from 50 years ago.
```{r echo=FALSE}
DT::datatable(Car_facts)
```
...
```{r}
gf_point(qsec ~ hp, data = Car_facts) %>%
  gf_lm(interval = "confidence")
model <- lm(qsec ~ hp, data = Car_facts)
...
model <- lm(qsec ~ hp, data = Car_facts)
M <- model.matrix(qsec ~ hp, data = Car_facts)
M
```
...
```{r daily-digital-18-Essay2, echo=FALSE}
etude::essay_response(
  prompt = "Explain how an `(Intercept)` vector relates to one of the coefficients in each of these functions."
)
```
...
```{r daily-digital-18-Essay3, echo=FALSE}
etude::essay_response(
  prompt = "Once you have constructed `M` and `b` as described above, use them to find the `\vec{x}` that satisfies `M\vec{x}≈\vec{b}`. Explain what `x` matches in the `model` created by `lm()`."
)
```
