...
For now, let's focus on the word "steps": plural. Generally, you cannot get to the desired destination with just one step. For instance, a pirate's treasure map might contain an instruction, "Dig 10 paces to the east of the tall tree." East is the direction of the vector. "Pace" is the length of the vector (about 3 feet). And 10 is the number of steps to take. 
...
Treating a vector as a single step, the mathematical representation of multiple or fractional steps is a ***scalar*** that multiplies the vector. The scalar---be it 17 or 0.36 or -1.5---tells you the length of the journey reported in units of the 1-step vector itself. In the previous paragraph, $\vec{p}$ is a single step; $10 \vec{p}$ is 10 steps, all taken in the direction of $\vec{p}$.

Any given vector may not point in the direction you want to travel. Often, then, a journey consists of stepping in the direction of one vector, then stepping in the direction of another vector that points in a different direction.

In a typical journey, there can be more than one vector involved. For instance, the vector pointing from the hidden cove to the tall tree---we'll call it $\overset{\longrightarrow}{\text{tree}}$---might be identified as direction north-west and length 1 nautical mile. The full directions to the treasure might then be, "Start from the hidden cove. Then march $3.2\, \overset{\longrightarrow}{\text{tree}} + 10\, \vec{p}$. 

Such a journey is a ***linear combination of vectors***. The vectors themselves are $\vec{p}$ and $\overset{\longrightarrow}{\text{tree}}$. The vector $3.2\, \overset{\longrightarrow}{\text{tree}}$ is a scaled version of $\overset{\longrightarrow}{\text{tree}}$; it points in the same direction as $\overset{\longrightarrow}{\text{tree}}$, but it has length 3.2 nautical miles. Similarly, $10\, \vec{p}$ is a vector of length 10 paces, pointing the the same direction of $\vec{p}$. When we scale a set of vectors and then add them, we have a linear combination, just as in Block 1 we created a linear combination of functions by scaling them and adding the results together. 
...
As you know, scaling a vector changes it's length. If the scalar is negative, the vector tip and tail are swapped. Figure \@ref(fig:single-scale) shows a vector $\vec{v}$ and, as a dotted line, all of the possible results of scaling the vector. Three locations on the dotted line are labeled (a), (b), and (c). The vectors we create by scalar multiplication can be placed anywhere we like. For the purpose of this illustration, imagine that all the vectors we are about to create have their tails at the tail of $\vec{v}$. The vector $-\frac{1}{2} \vec{v}$  will be half as long as $\vec{v}$ and point in the exact opposite direction. The tip of $-\frac{1}{2} \vec{v}$ will therefore be at the point marked (a). Similarly, the vector $\frac{4}{5} \vec{v}$ has its tip at (b) and $2.5 \vec{v}$ has the tip at (c). For any point on the dotted line, there is some numerical scalar $\alpha$ such that the scaled vector $\alpha \vec{v}$ will have its tip at that point.
...
```{r single-scale, echo=FALSE, fig.cap="With a suitable scalar multiplying the vector $\\vec{v}$, any point on the dotted line can be reached. The dotted line---extended to infinity---is the subspace spanned by the vector."}
Pts <- tibble(
  x = c(-2, 1/2, 4),
  y = c(1, -1.5, -5),
  lab = c("a", "b", "c")
...
gf_segment(2 + - 5 ~ (-3) + 4, color = "magenta", linetype="dotted") %>%
  Znotes::gvec(from=c(-1,0), to=c(1, -2), color="magenta", label="") %>%
