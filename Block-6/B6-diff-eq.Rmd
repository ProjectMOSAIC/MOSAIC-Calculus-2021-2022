...
$$\partial_t V = (1+r) V + M\ .$$
The left-hand side of this equation is boiler plate; there is a single state variable named $V$ and we are describing about how $V$ varies with time, that is, we are describing $\partial_t V$. The description itself is on the right side of the equation. The description take the form of a dynamical function, which we could generically call $g_V(V)$. The dynamical function always takes the set of state variables as inputs. In this example, there's just the one state variable $V$, so $g_V()$ has only one argument: $V$. Of course, there can also be parameters in the function, like $r$ and $M$ here.
...
$$\partial_t \theta(t)  =  g_\theta(\theta, v_\theta) =  v_\theta(t)\\
