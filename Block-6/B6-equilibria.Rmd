## Equilibrium and transient {#equilibria}
...
$$\require{cancel}\text{Simplification:}\ \ \  \ \ \begin{array}{cccc}\partial_t x  = &\cancel{ax} & + & b y\\\partial_t y = &c x& + & \cancel{dy}\end{array}\ .$$
We're going to modify the equations a little, specifically, we'll take the derivative with respect to time of both sides of the top equation, giving:
$$\begin{array}{cccc}\partial_{tt} x  = & &  & b\ \partial_{t\ } y\\\partial_{t\ } y = &c x&  & \end{array}\ .$$
...
What are $A$ and $B$? That depends on the initial condition. Evaluate both sides of the equation at $t=0$ to get $$x(0) = A e^{\lambda_1 0}+ B e^{\lambda_2 t} = A + B\ .$$
...
$$\partial_t x(t) = \lambda_1 A e^{\lambda_1 t} + \lambda_2 B e^{\lambda_2 t}\ .$$ Again, evaluate this at $t=0$ to get a second equation for the initial condition the pair being 
$$\begin{array}{rcrcr}x(0) & =& A & + &  B\\\partial_t x(0) & = &  \lambda_1 A & + &  \lambda_2 B\end{array}\ \ \ \ \implies\ \ \ \ 
 
 \left[\begin{array}{cc}1 & 1\\\lambda_1 & \lambda_2\end{array}\right] \left[\begin{array}{C}A\\ B\end{array}\right] = \left[\begin{array}{C}x(0)\\ \partial_t x(0)\end{array}\right] .$$
From Block 5, we know how to solve such matrix equations. So, given the initial values $x(0)$ and $\partial_t x(0)$, we can find an exact, quantitative solution to the differential equation. 

::: {.example data-latex=""}
Let's look at the flow field, some trajectories and their time series for the system $$\partial_t x = b y\\\partial_t y = c x$$ for $b=1$ and $c=2$.

```{r bc-system, echo=FALSE, fig.cap="Three trajectories from the system $$\\begin{array}{ccc}\\partial_t x = y\\\\\\partial_t y = 2 x\\end{array}$$"}
...
```{r bc-system-ts, echo=FALSE, fig.cap="The time series $x(t)$ for the three different trajectories in Figure \\@ref(fig:bc-system)."}
...
$$\left[\begin{array}{cc}1 & 1\\1.4142 & -1.4142\end{array}\right] \left[\begin{array}{C}A\\ B\end{array}\right] = \left[\begin{array}{C}x(0)\\ \partial_t x(0)\end{array}\right] .$$

```{r}
M <- cbind(rbind(1, 1.4142), rbind(1, -1.4142))
b_black = rbind(0.72, -1)
b_magenta = rbind(0.70, -1)
qr.solve(M, b_black)
...
For both trajectories, the initial amplitude of the decaying exponential is much larger than for the growing exponential. That's why the time series decay toward zero initially. But as $t$ grows, the exponential growth become much more important. For the black trajectory, the exponential growth has a positive coefficient, so the growth is toward $\infty$. But for the $\color{magenta}{\text{magenta}}$ trajectory, the exponential growth as a negative coefficient, thus that trajectory grows toward $-\infty$.
...
$$\begin{array}{cccc}\partial_t x  = &ax & + & b y\\\partial_t y = &c x& + & dy\end{array}\ .$$

Step 1: Differentiate with respect to $t$ both sides of the top equation, giving
$$\begin{array}{cccc}\partial_{tt} x  = &a\, \partial_t x & + & b\, \partial_t y\\\partial_t y = &c x& + & dy\end{array}\ .$$
