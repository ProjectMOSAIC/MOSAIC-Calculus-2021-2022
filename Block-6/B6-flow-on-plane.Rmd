...
```{r rf-nullclines, echo=FALSE, fig.cap="Flow field and nullclines for the rabbit/fox dynamics.", warning=FALSE, message=FALSE}
...
## Generic behaviors {#qualitative-stability}
...
```{r four-parts, echo=FALSE, fig.cap="Four quadrants of linear dynamics near the fixed point.", warning=FALSE, message=FALSE}
...
```{r rf-orbit, echo=FALSE, fig.cap="The rabbit/fox system has orbits that are neutrally stable.", message=FALSE, warning=FALSE}
...
We'll use $u$ and $w$ as the dynamical variables in the linear approximation in order to avoid confusion with the original, nonlinear equations. The linearized dynamics are therefore:
$$\partial_t u = \ \ w\\
\partial_t w = - u$$

```{r linearized-pendulum, echo=FALSE, fig.cap="Flow field, nullclines, and trajectories of the pendulum (black) and the linearized pendulum ($\\color{magenta}{\\text{magenta}}) from three different initial conditions.", warning=FALSE, message=FALSE}
tnl0 <- integrateODE(dtheta ~ v, dv ~ - sin(theta), theta=1, v = 0, tdur=6)
tnl1 <- integrateODE(dtheta ~ v, dv ~ - sin(theta), theta=2, v = 0, tdur=6)
tnl2 <- integrateODE(dtheta ~ v, dv ~ - sin(theta), theta=2.8, v = 0, tdur=6)
t0 <- integrateODE(dtheta ~ v, dv ~ - theta, theta=1, v = 0, tdur=6)
t1 <- integrateODE(dtheta ~ v, dv ~ - theta, theta=2, v = 0, tdur=6)
t2 <- integrateODE(dtheta ~ v, dv ~ - theta, theta=2.8, v = 0, tdur=6)

contour_plot(-sin(theta) ~ theta & v, contours_at=0, contour_color="blue", , domain(theta=-3:3, v=-2.5:2.5), labels=FALSE) %>%
  contour_plot(v ~ theta & v, contours_at=0, contour_color="red", labels=FALSE) %>%
vectorfield_plot(v ~ theta & v, -sin(theta) ~ theta & v, domain(theta=-3:3, v=-2.5:2.5)) %>%
...
`r insert_calcZ_exercise("50.05", "ePqij6", "Exercises/finch-show-hamper.Rmd", skill="N-SCL6M")`

`r insert_calcZ_exercise("50.07", "3A3pMv", "Exercises/bee-wake-bottle.Rmd",skill="N-SCL6M")`

`r insert_calcZ_exercise("50.09", "KzgT1c", "Exercises/crow-find-sofa.Rmd", skill: "X-HGCV2")`

<!-- identifying nullclines --> 
`r insert_calcZ_exercise("50.11", "owE1WY", "Exercises/eagle-choose-mattress.Rmd",skill="X-HGCV2")`

<!-- linearization of rabbit-fox system -->
`r insert_calcZ_exercise("50.13", "3Ug3IH", "Exercises/cow-sharpen-table.Rmd", skill="N-CB2LE")`

`r insert_calcZ_exercise("50.15", "s5x504", "Exercises/bear-give-futon.Rmd", skill="N-CB2LE")`

<!-- optimization by gradient ascent -->

`r insert_calcZ_exercise("50.17", "iRZfBY", "Exercises/walnut-tell-pot.Rmd")`
