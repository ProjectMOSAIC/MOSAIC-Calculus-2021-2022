...
$$\partial_{tt} A \sin(\omega t) = - \frac{k}{m}\, A \sin(\omega t)$$ 
Differentiating $\sin(\omega t)$ once let's us re-write the left-hand side of the equation in terms of a *first derivative*

$$\partial_{t} A \omega\, \cos(\omega t) = - \frac{k}{m}\, A \sin(\omega t)$$
Differentiating again gives
$$- \omega^2 A\sin(\omega\, t) = - \frac{k}{m}\, A\sin(\omega t)\ .$$
Simplifying this by cancelling out the $A \sin(\omega t)$ term gives $\omega^2 = \frac{k}{m}$, where $\omega$ is the angular frequency of the oscillation.

::: {.takenote data-latex=""}
Instead of using $A \sin(\omega t)$ as the *ansatz* we could have used $A \sin(\omega t) + B \cos(\omega t)$. Working through this ansatz would produce exactly the same result, that $\omega^2 = \frac{k}{m}$. So the solution to the spring-mass system will be, in general, a linear combination of the sine and the cosine functions with angular frequency $\omega$.
:::

## Exponential or sinusoid?

In Chapter \@ref(eigenvalues), we established that solutions to second-order linear differential equations have the form $m_1 e^{\lambda_1 t} + m_2 e^{\lambda_2 t}$. Yet in the previous section, we saw one linear second-order differential equation, $\partial_{tt} y = - \omega^2 y$ where the solution is a linear combination of a sine and a cosine function: $y(t) = A \sin(\omega t) + B \cos(\omega t)$ with $\omega = \sqrt{\frac{k}{m}}$.
...
$$\partial_{tt} y = -a\, \partial_t y -b y\ ,$$ where $b$ would be the positive number $\frac{k}{m}$ and $a$ is another positive number reflecting the magnitude of friction.
...
This form suggests, at least  to the avid reader of the previous chapter, that we look for a solution $y(t) = m_1\, e^{\lambda_1\, t} + m_2\, e^{\lambda_2\, t}$ in terms of the eigenvectors and eigenvalues of the matrix $\left[\begin{array}{cc}a & b\\1 & 0\end{array}\right]$.
...
This is a quadratic polynomial in $\lambda$, so we can use the "quadratic formula" to find values for $\lambda$ that are consistent with the parameters $a$ and $b$. In applying the quadratic formula you have to remember that the standard statement is for the roots of $a x^2 + b x + c = 0$ and make adjustment for the fact that our polynomial uses the parameter names differently: $\lambda^2 + a \lambda + b = 0$.
...
$$\partial_{tt} y = -a\, \partial_t y -b y\ $$ to be a model of a damped spring-mass system, the parameters $a$ and $b$ both have to be positive. There are important consequences to this for the eigenvalues.

FILL IN THIS PART. If the spring is weak compared to friction, then $b$ is small, and the eigenvalues will both be real. No oscillation, "over-damped"

If the friction is weak compared to the spring, then $b$ is large and the eigenvalues will have a real and an imaginary part. WORK THROUGH $e^{\lambda t}$ to show that it is $e^{kt} e^{i\omega t}$: an exponentially decaying sinusoid.

::: {.example data-latex=""}
Special case: $a^2 = 4 b$. "Critically damped" Show the solution relative to the "over-damped" case using `integrateODE()`.
:::
...
`r insert_calcZ_exercise("XX.XX", "CMBvRg", "Exercises/tiger-sharpen-fork.Rmd")`

`r insert_calcZ_exercise("XX.XX", "IVEjiX", "Exercises/falcon-shut-hamper.Rmd")`

`r insert_calcZ_exercise("XX.XX", "P4xpYP", "Exercises/pine-bring-window.Rmd")`

`r insert_calcZ_exercise("XX.XX", "Yn47ct", "Exercises/cat-sleep-door.Rmd")`

`r insert_calcZ_exercise("XX.XX", "iX26Ra", "Exercises/falcon-have-mattress.Rmd")`
