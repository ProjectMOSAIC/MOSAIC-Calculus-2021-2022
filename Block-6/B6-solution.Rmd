...
POINT OUT THAT "finding a solution" should really be "finding a trajectory." The word "solution" incorrectly suggests that it is the end-all and be-all of working with differential equations. Instead, finding trajectories is one tool for understanding a dynamical system. If the particular goal of your modeling is to make a prediction of the future state given the present state, then the "solution" does indeed give you what you are looking for. But for other modeling goals, the "solution" does not give you what you need.
...
gf_segment(0.217 + 0.283 ~ 1.9865 + 2.0125, color="black",
 ...
 gf_lims(y = c(.1,.3), x = c(1.85,2.05)) %>%
  gf_labs(y = "Fox density", x = "Rabbit density", subtitle="A smal part of Rabbit/Fox state space") %>%
  gf_refine(coord_fixed())
```
To draw the entire flow field, repeat this process at many other points in the state space as in Figure \@ref(fig:rf-flow-field).

```{r rf-flow-field, echo=FALSE, fig.cap="Left: The flow field depicted by drawing the state velocity vector (times $dt = 0.1$) for many points in the state space. Right: Instead of plotting the state velocity vector, small snippets of trajectories of duration 0.1 are shown. The curvature of the trajectories can help to envision the flow more precisely.", out.width="50%", fig.show="hold"}
vectorfield_plot(0.66*r - 1.33*r*f ~ r & f,
                 -f + r*f ~ r & f, 
                 domain(r=0.1:3, f=0.1:1), transform=I,
                 npts=15) %>%
...
Some people prefer a visualization of short segments of actual trajectories, as in the right panel in Figure \@ref(fig:rf-flow-field), rather than the state velocity vector. This is a matter of personal preference.
...
```{r calcplot3d1, echo=FALSE, out.width="1%", fig.cap="The flow field for the rabbit/fox dynamics. Click at an initial state to generate the trajectory from that state. You may need to pinch in or out to see the flow arrows clearly."}
...
```{r fox-rabbit-ts, fig.cap="Times series of the rabbit and fox densities.", out.width="50%", fig.show="hold"}
slice_plot(Eco_soln$r(t) ~ t, domain(t=0:5)) %>% gf_labs(title="Rabbits")
slice_plot(Eco_soln$f(t) ~ t, domain(t=0:5)) %>% gf_labs(title="Foxes")
```
...
$$\frac{x}{1-x} = \underbrace{e^C}_{A} e^t\  .$$ Since $e^C$ is just a constant, we'll write it more simply as $A$.
...
Not all differential equations can be separated in this way, and even for those that can, the integrals may not be tractable. So this route to a solution is not a general-purpose one, unlike the Euler method. Still, the Euler method gives only an approximate solution, so with Euler we need to take care that the approximation is close enough for the purpose at hand. In this case, we have both an Euler solution (with $dt=0.1$) and a symbolic solution. Figure \@ref(fig:diff-Euler-separation) shows the *difference* between the two solutions, which ideally should be zero. To show more of the time domain of the solution, we'll reset the initial condition to $x_0 = 0.01$. This corresponds to $A = 1/99$.

```{r diff-Euler-separation, echo=FALSE, fig.cap="The difference between the Euler and the symbolic solution to $\\partial_t x = x (1-x)$ as a fraction of the symbolic solution. At the worst, the Euler solution is off by 1.5 parts in one-million."}
Euler <- integrateODE(dx ~ x*(1-x), x=0.01, tdur=list(from=0, to=20, dt=0.1))
slice_plot((Symb_soln(t, A=1/99) - Euler$x(t))/Symb_soln(t, A=1/99) ~ t, domain(t=0:20))
```

## Exercises

`r insert_calcZ_exercise("XX.XX", "Z3Mq3G", "Exercises/rabbit-grow-candy.Rmd")`

`r insert_calcZ_exercise("XX.XX", "Ul19SO", "Exercises/turtle-show-sofa.Rmd")`

`r insert_calcZ_exercise("XX.XX", "jd6HE8", "Exercises/ape-grow-oven.Rmd")`

`r insert_calcZ_exercise("XX.XX", "5DVPWG", "Exercises/spruce-make-lamp.Rmd")`
