...
Figure \@ref(fig:slope-at-point) shows the sinusoid function. Zooming in on the region near each colored dot shows that, locally, the function is approximately a straight-line function. The slope of that straight-line (drawn as a segment near the dot) is the slope of the sinusoid at that point. Like the sinusoid itself, the slope of the sinusoid oscillates periodically from negative to positive.

```{r slope-at-point, echo=FALSE, fig.cap="Zooming in near a specific input value  makes the function look like a straight line. The slope at that input value is the slope of that straight line."}
Pts <- tibble(
  x = c(-2, 1),
  y = sin(x),
  color = c("magenta", "green")
)
Pa <- slice_plot(sin(x) ~ x, domain(x=c(-5,5))) %>%
  gf_hline(yintercept=~0, color="magenta", linetype="dashed") %>%
  gf_point(y ~ x, color=~color, size=5, alpha=0.3, data=Pts) %>%
  gf_segment(.85 + .958 ~ .9 + 1.1, color="green") %>%
  gf_segment(-0.92 + -0.962 ~ -2.05 + -1.95, color="magenta") %>%
  gf_theme(scale_color_identity())
Pb <- slice_plot(sin(x) ~ x, domain(x=-2.25:-1.75)) %>%
  gf_point(y ~ x, color=~color, size=20, alpha=0.3, data=Pts[1,]) %>%
  gf_theme(scale_color_identity()) %>%
  gf_segment(-0.9 + -0.942 ~ -2.1 + -2, color="magenta") %>%
  gf_labs(y="")
Pc <- slice_plot(sin(x) ~ x, domain(x=.75:1.25)) %>%
  gf_point(y ~ x, color=~color, size=20, alpha=0.3, data=Pts[2,]) %>%
  gf_theme(scale_color_identity()) %>%
  gf_segment(.825 + .879 ~ .9 + 1, color="green") %>%
  gf_labs(y="")
gridExtra::grid.arrange(Pa, Pb, Pc, ncol=3)
```
